...
from modules import RNNEncoder, SimpleSoftmaxLayer, BasicAttn, BiAttn, CoAttn, BiLSTMEncoder
...
class QAModel(object):
    ...
    def build_graph(self):
        ...
        _, C2Q_attn_output, _, Q2C_attn_output = attn_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens, self.context_mask) # attn_output is shape (batch_size, context_len, hidden_size*2)

        # Concat attn_output to context_hiddens to get blended_reps
        blended_reps = tf.concat([context_hiddens, C2Q_attn_output, tf.multiply(context_hiddens, C2Q_attn_output), tf.multiply(context_hiddens, Q2C_attn_output)], axis=2) # (batch_size, context_len, hidden_size*8)
        """
        
        """CoAttn"""
        # Use context hidden states to attend to question hidden states
        attn_layer = CoAttn(self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2, self.FLAGS.batch_size)
        _, C2Q_attn_output, _, Q2C_attn_output, Co_attn_output = attn_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens, self.context_mask) # attn_output is shape (batch_size, context_len, hidden_size*2)

        # Concat C2Q_attn_output to Co_attn_output to get blended_reps
        blended_reps = tf.concat([Co_attn_output,C2Q_attn_output], axis=2) # (batch_size, context_len, hidden_size*4)
        # biLSTM
        biLSTM_encoder = BiLSTMEncoder(self.FLAGS.hidden_size, self.keep_prob)
        blended_reps = biLSTM_encoder.build_graph(blended_reps,self.context_mask)
